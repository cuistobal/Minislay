...
/*   Updated: 2025/02/08 16:10:46 by chrleroy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int parse_command_list(t_tokn **current) {
    if (!parse_command(current)) return 0;
...
int parse_simple_command(t_tokn **current) {
    ...
    if (!match(current, "WORD", NULL)) return 0;  // Command name
...
int parse_assignment(t_tokn **current) {
    ...
    if (match(current, "WORD", NULL) && match(current, "OPERATOR", "=")) {
        if (parse_expression(current)) return 1;
    }
    *current = save;
    return 0;
}
